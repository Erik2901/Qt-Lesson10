...
WorkClass::WorkClass(QObject *parent)
    : QObject{parent}
{
    file = new QFile("../Qt-HomeWork10/res/sw.txt");
...
bool WorkClass::saveWork()
{
    if(WorkName.size() != 0){
        QTextStream stream(file);
        QString data = "Work Name: " + WorkName + ", Process: " + QString::number(process) + ", End Date: " + QString::number(day) + "." + QString::number(month) + "." + QString::number(year) + "\n";
        stream << data;
        (count)++;
        return true;
    }
